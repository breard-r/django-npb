...
class Paste(models.Model):
    ...
    created_on = models.DateTimeField(
        auto_now_add=True, blank=True, verbose_name=_("created on")
    )
    edited_on = models.DateTimeField(
        auto_now=True, blank=True, verbose_name=_("edited on")
    )
    ...
    author_ip = models.GenericIPAddressField(
        null=True, blank=True, verbose_name=_("author's IP address")
    )
    lexer = models.CharField(
        max_length=128, choices=LEXERS, blank=True, verbose_name=_("language")
    )
    title = models.CharField(
        max_length=256, default="", blank=True, verbose_name=_("title")
    )
    ...
    def formated_content(self):
        ...
        lexer = None
        lexer_options = {"stripall": True, "tabsize": tabsize}
        if self.lexer != "":
...
class Report(models.Model):
    ...
    reporter_ip = models.GenericIPAddressField(
        null=True, blank=True, verbose_name=_("reporter's IP address")
    )
    reported_on = models.DateTimeField(
        auto_now_add=True, blank=True, verbose_name=_("reported on")
    )
    paste = models.ForeignKey(
        Paste, on_delete=models.CASCADE, verbose_name=_("reported paste")
    )
    category = models.CharField(
        max_length=16, choices=REPORT_CATEGORIES, verbose_name=_("category name")
    )
